...
1. Скачайте подходящий `swilib-types-PLATFORM.h` из [Swilib data types for disassembler](https://devtools.siepatch.dev/re#swilib-types)
...
1. Скачайте соответствующий `cpu-PHONE.txt` или `cpu-pmb887x.txt` из [CPU IO registers](https://devtools.siepatch.dev/re#cpu-registers)
...
1. Скачайте подходящий `symbols-PHONE.txt` из [Firmware symbols for disassembler](https://devtools.siepatch.dev/re#swilib-symbols)
2. Откройте `Window -> Script Manager -> ImportSymbolsWithDataType.java -> Run Script`
