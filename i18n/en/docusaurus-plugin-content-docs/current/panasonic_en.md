...
3. If nothing happens, the code was not entered quickly enough. Try again.
...
3. Theoretically, Bluetooth (if available) and IrDA could work, but we have not tested them.
...
# Service software
Infineon's own PhoneTool can be used. Versions 50 and 60 complement each other: 60 works for the Audio tab and RAM read/write, 50 handles everything else.
...
## Installation
- x50: Install, replace `dwdio.dll` with a patched version in the installation folder, then use it.
- x60: Extract, import registry entries from `x60factory-registri.zip\m\m\x60.reg`, then use it.

## Usage
After launching, specify the required COM port in `Settings`.
Connect using the Update info button. If it does not work immediately, try pressing `V24 AT# on/off` a few times.
...
npx tsx examples/dwd-apoxi-memory-dump.js \
  ...
  --size 0x4000000 \      # Read length: 64 MB
  --out ./sa7-vq24.bin \  # Output file name
...
With [x65flasher](https://fw.fasoley.net/?file=panasonic/x65Flasher-2103.rar)  it is simpler: select the "Phone with entered SKEY" loader and work at the same speeds.

# Flashing a newer FW version
Only possible via UART.
First, back up your EEPROM it contains RF and battery calibrations. Use PhoneTool x50 with the appropriate EEPROM config file (for VS7/SA7/MX7, it is [leopard_eep150.cfg](https://fw.fasoley.net/?file=panasonic/eeprom/leopard_eep150.cfg)) in PTEST mode. 
