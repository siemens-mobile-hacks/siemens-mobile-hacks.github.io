---
title: "V_KLay"
sidebar_label: "V_KLay"
sidebar_key: "soft.flashers.v-klay"
---
{/* Generated by tools/generateSoftware.ts. Do not edit manually. */}

# V_KLay

**Homepage:** [http://vi-soft.com.ua/](https://web.archive.org/web/20130813164842/http://vi-soft.com.ua/) (web archive)<br/>
**Author:** ValeraVi (Valeriy Vishnyak)

A program for applying patches, reading/writing Flash, and changing the keyboard
layout of Siemens mobile phones.

Features:

- Apply and remove VKP patches with verification of old data;
- Create, open, edit, and save VKP patches;
- Search and replace in patch text;
- Read and write phone memory;
- Save read data to a file and load data from a file;
- Work with the entire fullflash, individual memory areas, and EEPROM;
- Apply patches directly to the phone or to a Flash file;
- Read, edit, and write keyboard layouts;
- Load and save layouts in VKL files;
- Work with VKD loaders;
- Automatically recognize patches, layouts, and Flash files passed via the
  command line;
- Control patch application and removal from the command line;
- Connect via Password boot, Chaos BootPatch, Bootcore bug, Patched
  bootcore, and Test point;
- Work with the phone without powering it off in online mode;
- Multilingual interface.

Support for a specific phone is determined by the availability of a suitable
VKD loader.

## Versions

**Current versions**

- **3.3** — [v_klay_setup.zip](https://git.siepatch.dev/siepatch/soft/media/branch/main/catalog/flashers/v-klay/files/v_klay_setup.zip)<br/>Windows 32-bit · 1.61 MiB

<details>
<summary><strong>Archived versions (4)</strong></summary>

- **2.7.2** — [v_klay_setup_v272.zip](https://git.siepatch.dev/siepatch/soft/media/branch/main/catalog/flashers/v-klay/files/v_klay_setup_v272.zip)<br/>Windows 32-bit · 2.50 MiB
- **2.5** — [v_klay_setup_v25.zip](https://git.siepatch.dev/siepatch/soft/media/branch/main/catalog/flashers/v-klay/files/v_klay_setup_v25.zip)<br/>Windows 32-bit · 2.25 MiB
- **2.1.12** — [v_klay_update_v2112.zip](https://git.siepatch.dev/siepatch/soft/media/branch/main/catalog/flashers/v-klay/files/v_klay_update_v2112.zip)<br/>Windows 32-bit · 1.28 MiB
- **2.1.5.2** — [v_klay_setup_v2152.zip](https://git.siepatch.dev/siepatch/soft/media/branch/main/catalog/flashers/v-klay/files/v_klay_setup_v2152.zip)<br/>English-language version<br/>Windows 32-bit · 2.22 MiB

</details>